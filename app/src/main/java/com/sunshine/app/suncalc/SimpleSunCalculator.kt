...
class SimpleSunCalculator : SunCalculator {
    override suspend fun calculateSunPosition(
        ...
        val meanObliqEcliptic =
            23 + (26 + ((21.448 - julianCentury * (46.8150 + julianCentury * (0.00059 - julianCentury * 0.001813)))) / 60) / 60
        ...
        val azimuth =
            if (azimuthDenom.absoluteValue() > 0.001) {
                val azimuthRad =
                    acos(
    ...
    override suspend fun calculateSunrise(
        location: GeoPoint,
        date: LocalDate,
    ): LocalTime? = calculateSunEvent(location, date, isSunrise = true)

    override suspend fun calculateSunset(
        location: GeoPoint,
        date: LocalDate,
    ): LocalTime? = calculateSunEvent(location, date, isSunrise = false)
